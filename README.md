...
剖析发现组装慢的主因是 `apply_native_regex_batch`（每次组装对全部消息跑正则脚本，Rust `regress` 回溯引擎对复杂脚本集可能 15-30 秒/批）。该调用的输出是 `(消息文本, 已按深度过滤的脚本集)` 的纯函数——**相同输入必然相同输出**。v3.3.0 在 invoke broker 层对批处理结果做 LRU 记忆化：
...
1. **启动通知**：加载成功时弹 toast「前端 Token 估算已启用（v3.3.0）」
